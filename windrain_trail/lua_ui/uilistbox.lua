...
local function new( ctrl )
	if (type(ctrl) ~= "userdata") then error("listbox expected") end
	...
	local setBorder = function( border, margin )
		return ctrl:set_border( border, margin )
	end
	...
	local getSelectedIndex = function( nPreviousSelected )
		return ctrl:get_selected_index( nPreviousSelected )
	end
...
-- export ----
_new_uilistbox_tb = new

LBS_MULTISELECTION = toDWORD('00000001')	 -- multi selection
LBS_ICON	= toDWORD('00000002')		--icon list

...
struct VS_OUTPUT {
	...
	float2 uv		: TEXCOORD0;
	float3 pos		: TEXCOORD1; //world pos
};
...

{
   ...
   float3 V = normalize( pos.xyz-_vecEye.xyz ); // V = position-eye
	...
	Out.Specular=vI*0.8; //Inscattering TNT2 doesn't support Specular arg, so output inscattering to Diffuse
...
float4 mainps(VS_OUTPUT i) : COLOR0
{
	...
	float2 uv=(i.pos.xy-float2(12000,20000))*0.0001;
	//float2 uv=(i.pos.xy)*0.0001;
	float factor=tex2D(texnoise,uv).r;
	factor+=tex2D(texnoise,uv*20).r/4;
	color.rb *= 0.7+factor/3;

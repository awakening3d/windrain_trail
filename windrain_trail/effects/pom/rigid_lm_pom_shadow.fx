...
struct VS_OUTPUT {
   ...
   float2 vParallaxOffsetTS : TEXCOORD6;   // Parallax offset vector in tangent space
...
VS_OUTPUT mainvs(float4 pos: POSITION, float3 n: NORMAL, float2 uv: TEXCOORD0, float3 bin: BINORMAL, float3 tan: TANGENT, float2 uv1: TEXCOORD1)
{
    ...
    o.vParallaxOffsetTS = vParallaxDirection * fParallaxLength;
       
    // Need to scale the amount of displacement to account for different height ranges
    // in height maps. This is controlled by an artist-editable parameter:
    o.vParallaxOffsetTS *= g_fHeightMapScale;
...
float4 mainps(VS_OUTPUT i, uniform bool bPS20=false, uniform bool bPS30=false) : COLOR0
{
   ...
   if (bPS30) { // pom
	...
	if (_bBlendPass) {
		...
		diffuseL += tex2D(texlightmap, i.uv.zw) + _vAmbientColor;
	}

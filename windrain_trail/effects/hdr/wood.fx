...
texture tTX1;
sampler basemap=sampler_state { Texture = <tTX1>; };
...
{
	float4 vBase = tex2D(basemap,tc0);
	...
	float diffuse=0.5f+dot(tc2.xyz,float3(0,1,0))*0.5;

	return vBase*diffuse+vSample*reflectionStrength;
}

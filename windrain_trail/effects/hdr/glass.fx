...
float4 mainps (float4 tc0 : TEXCOORD0, float4 tc1 : TEXCOORD1, float4 tc2 : TEXCOORD2, float4 tc3 : TEXCOORD3, float4 tc4 : TEXCOORD4) : COLOR
{
	...
	if (1==gMode) {
		reflection.rgb=DecodeRGB16(reflection);
		refraction.rgb=DecodeRGB16(refraction);
	} else if (2==gMode) {
		reflection.rgb=DecodeRGBE8(reflection);
		refraction.rgb=DecodeRGBE8(refraction);
	}
	...
	float4 base=float4(0.8,1,0.8,1);

	return reflection/2 + base*reflection/2 +refraction*base;//+base/4;
}
